...
# -----------------------------------------------------------
echo ""
echo "[6/7] ML models (~1.4GB):"
echo "  - BAAI/bge-m3 (embedding, ~1.4GB)"
echo "  * Reranking uses Cohere API (no local download needed)"
echo ""
read -p "  Download embedding model now? [y/N] " -n 1 -r
echo ""
if [[ $REPLY =~ ^[yY]$ ]]; then
    echo "  Downloading embedding model..."
    # You might want to update download_models.py to only download embedding
    python3 backend/scripts/download_models.py
else
    echo "  Skipped. Model will be downloaded on first use."
fi
