...
Mỗi department có 1 workspace duy nhất.
- Admin: thấy tất cả workspaces
- User: thấy workspace của department mình
"""
...
async def _get_or_create_department_workspace(db: AsyncSession, department: Department) -> KnowledgeBase:
    """
    Get or create workspace for a department.
    Mỗi department có 1 workspace duy nhất.
    """
    # Check if workspace already exists for this department
    result = await db.execute(
        select(KnowledgeBase).where(KnowledgeBase.department_id == department.id)
    )
    ...
    # Create new workspace for department
    kb = KnowledgeBase(
        name=f"KB {department.name}",  # e.g., "KB Kế toán"
        description=f"Kiến thức của phòng {department.name}",
...
async def list_workspaces(
    ...
    - Admin: thấy tất cả workspaces
    - User: thấy workspace của department mình + tất cả public docs (thông qua hybrid search)
    """
    if current_user.role == "Admin":
        # Admin thấy tất cả workspaces
        result = await db.execute(
            select(KnowledgeBase).order_by(KnowledgeBase.department_id, KnowledgeBase.updated_at.desc())
        )
        kbs = result.scalars().all()
    else:
        # User thường: chỉ thấy workspace của department mình
        if current_user.department_id:
            ...
            if department:
                kb = await _get_or_create_department_workspace(db, department)
                kbs = [kb]
            else:
                kbs = []
        else:
...
async def get_suggested_questions(
    ...
    Questions are generated based on the newest indexed document.
    """
    ...
    # Generate using the shared service
