...
export default function Knowledge() {
    ...
    useEffect(() => {
        ...
        ragGraphApi.getGraph(selectedWs.id)
            ...
            .then(data => {
                ...
                // Map API response to react-force-graph format
                const nodes = (data.nodes || []).map(entry => ({
                    id: entry.id,
                    name: entry.label,
                    type: entry.entity_type || 'Unknown',
                    val: entry.degree ? Math.max(1, Math.min(entry.degree / 2, 20)) : 1, // Node size based on degree
                ...
                // Filter out links where source/target don't exist in nodes
                const nodeIds = new Set(nodes.map(n => n.id));
                const validLinks = links.filter(l => nodeIds.has(l.source) && nodeIds.has(l.target));

                // Add neighbors to nodes for highlighting
                const graph = { nodes, links: validLinks };
                
                setGraphData(graph);
                
                // Automatically center after loading
                setTimeout(() => {
    ...
    useEffect(() => {
        if (fgRef.current && graphData.nodes.length > 0) {
            // Giảm repulsion để các node gần nhau hơn
            fgRef.current.d3Force('charge').strength(-40);
            // Giảm link distance để kéo gần lại
    ...
    const handleNodeClick = useCallback(node => {
        if (fgRef.current && node) {
            // Focus camera (centerAt and zoom like the guide)
    ...
    const paintNode = useCallback((node, ctx, globalScale) => {
        ...
        // Use translucent gray when dimmed like in the guide
        ...
        // Draw label if zoomed in (like in the guide)
        if (globalScale > 1.5 || isHighlighted) {
            ...
            if (highlightNodes.size > 0 && !isHighlighted) {
                ctx.fillStyle = isDarkMode ? 'rgba(150,150,150,0.3)' : 'rgba(156, 163, 175, 0.4)'; // Dimmed
            } else {
                ctx.fillStyle = isDarkMode ? 'rgba(255, 255, 255, 0.85)' : 'rgba(17, 24, 39, 0.85)'; // Normal
            }
        ...
        <div className="flex flex-col h-full bg-white dark:bg-gray-900 overflow-hidden relative p-1 lg:p-2 gap-2 lg:gap-3">
            ...
            <div className="bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2 flex items-center justify-between shrink-0 shadow-sm">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-sm font-bold text-gray-900 dark:text-white tracking-wide">Sơ đồ tri thức</h1>
                </div>
            ...
            <div className="flex-1 relative flex bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm" ref={containerRef}>
                {loading ? (
                    ...
                    <>
                        ...
                        {/* Instruction Overlay based on Guide */}
